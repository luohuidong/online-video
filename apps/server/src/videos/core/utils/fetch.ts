...
export async function fetchJson<T = unknown>(url: string, ms = 60000): Promise<T | null> {
  try {
...
export async function fetchJsonOrThrow<T = unknown>(url: string, ms = 60000): Promise<T> {
  const res = await timedFetch(url, ms);
